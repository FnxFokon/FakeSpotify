...
const SearchView = () => {
    ...
    const { searchSong, searchArtist, searchAlbum } = useSelector(selectAlbumData)
    const { activeSong, isPlaying } = useSelector(state => state.player)
    //on récupère les data de notre selector
    const dataAlbum = searchAlbum ? searchAlbum['hydra:member'] : [];
    const dataArtist = searchArtist ? searchArtist['hydra:member'] : [];
    const dataTitle = searchSong ? searchSong['hydra:member'] : [];
        ...
        <>
            ...
            <div className='flex flex-wrap'>
                {dataAlbum && dataAlbum.map((data, index) => {
                    return (
                        ...
                        <div className='p-3 m-3' key={`album${index}`}>
                            <AlbumCard
                                data={data}
                                songs={data.songs}
                                isPlaying={isPlaying}
            ...
            <div className='flex flex-wrap'>
                {dataArtist && dataArtist.map((data, index) => {
                    return (
                        //on appelle ArtistView en lui passant les props
                        <div className='p-3 m-3' key={`title${index}`}>
                            <ArtistView dataArtist={data} />
                        </div>
                    )
                })}
            </div>
